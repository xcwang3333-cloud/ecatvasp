...
def _materialized_case(
    ...
    sources: tuple[ReactionEnergySource, ...] = (
        clean,
        ads,
        _che_source(baseline_conditions),
    )
    ...
    bindings = (clean_binding, ads_binding)
    ...
    return project, sources, bindings, durable
...
def test_source_receipt_must_match_bound_analysis_identity(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        ReactionDiagramError,
        match="receipt differs from Analysis parameters_hash",
    ):
        materialize_reaction_diagram(
            project_root=tmp_path,
            definition=_pathway(),
...
def test_reopen_reconciliation_propagates_source_hash_drift(tmp_path: Path) -> None:
    ...
    store.save(bundle)
    assert durable.analysis.parameters_hash is not None
    requirement = ThermochemistryAnalysisRequirement(
        ...
        parameters_hash=durable.analysis.parameters_hash,
    )
...
def _calculation(project: Project, recipe_id: str) -> Calculation:
    return Calculation(
        project_id=project.id,
...
def _workflow_gate_evidence(
    *,
    ...
    binding_a = WorkflowStepBinding(
        ...
        resolved_input_structure_snapshot_id=first.input_structure_snapshot_id,
    ...
    binding_b = WorkflowStepBinding(
        ...
        resolved_input_structure_snapshot_id=second.input_structure_snapshot_id,
        materialization_reason="test",
    )
    selections = (
        WorkflowBindingSelection(
            ...
            current_calculation=first,
        ),
        WorkflowBindingSelection(
            step_key="freq-b",
            current_binding=binding_b,
            current_calculation=second,
        ),
    )
    gates = tuple(
        WorkflowStepGate(
            ...
            calculation_id=(
                item.current_calculation.id if item.current_calculation else None
            ),
...
def test_multi_anchor_workflow_gate_blocks_historical_generation(tmp_path: Path) -> None:
    project, _, bindings, durable = _materialized_case(tmp_path)
    first = _calculation(project, "test.frequency.a")
    second = _calculation(project, "test.frequency.b")
    workflow_gates = _workflow_gate_evidence(first=first, second=second)
    assert durable.analysis.parameters_hash is not None
    requirement = ThermochemistryAnalysisRequirement(
        key="diagram",
        project_id=project.id,
    ...
    replacement = replace(second, id=new_calculation_id())
    historical = reconcile_thermochemistry_analyses(
        requirements=(requirement,),
        analyses=analyses,
        artifacts=artifacts,
        dependencies=durable.dependency_records,
        workflow_gates=_workflow_gate_evidence(first=first, second=replacement),
    ).requirement("diagram")
