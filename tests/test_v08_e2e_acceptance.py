        ...
        method=MethodDefinition(
            ...
            potcars=tuple(
                PotcarIdentity(
                    element=element,
                    symbol=element,
                    sha256=hashlib.sha256(element.encode()).hexdigest(),
                )
...
def _surface_snapshot(*, with_hydrogen: bool) -> StructureSnapshot:
    ...
    return StructureSnapshot(
        lattice=Lattice(
            ((10.0, 0.0, 0.0), (0.0, 10.0, 0.0), (0.0, 0.0, 18.0))
        ),
        sites=tuple(sites),
...
def _parsed_frequency_source(
    *,
    ...
    assert len(wavenumbers) == 3 * len(atom_uids)
    result = VaspResultDocument(
        calculation_type=calculation_type,
        sources=(
            VaspResultSource(
...
def test_v08_final_e2e_reopen_requested_conditions_and_source_drift(
    tmp_path: Path,
) -> None:
    ...
    diagram = materialize_reaction_diagram(
        ...
        source_bindings=(
            ReactionSourceArtifactBinding(
                "star", clean_thermo.analysis, clean_thermo.artifact
            ),
            ReactionSourceArtifactBinding(
                "H_star", ads_thermo.analysis, ads_thermo.artifact
            ),
            ReactionSourceArtifactBinding(
                "H2", h2_thermo.analysis, h2_thermo.artifact
            ),
        ),
        baseline_conditions=baseline_conditions,
    ...
    assert {item.id for item in reopened.analyses} == {item.id for item in bundle.analyses}
    assert {item.id for item in reopened.artifacts} == {item.id for item in bundle.artifacts}
