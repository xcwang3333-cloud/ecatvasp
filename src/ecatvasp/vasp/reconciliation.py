...
def reconcile_generated_input_directory(
    *,
    ...
    if not manifest_path.is_file():
        _fail(
            VaspFailClosedCode.INPUT_MANIFEST_MISSING,
            "input-manifest.json is required",
        )
    ...
    if preparations.get("poscar_sha256") != prepared_poscar.sha256:
        _fail(
            VaspFailClosedCode.GENERATED_POSCAR_MISMATCH,
            "manifest POSCAR hash does not match reconciled POSCAR",
        )
    if preparations.get("incar_sha256") != expected_incar.sha256:
        _fail(
            VaspFailClosedCode.INPUT_MANIFEST_IDENTITY_MISMATCH,
            "manifest INCAR hash does not match recompiled INCAR",
        )
...
def _validate_files(
    *,
    ...
    kpoint_payload = _require_dict(manifest, "kpoints")
    uses_kpoints_file = kpoint_payload.get("uses_kpoints_file")
    if not isinstance(uses_kpoints_file, bool):
        _fail(
            VaspFailClosedCode.INPUT_MANIFEST_INVALID,
            "kpoints.uses_kpoints_file must be boolean",
        )
    ...
    *,
    ...
    for expected_poscar_index, raw in enumerate(entries):
        if not isinstance(raw, dict):
            _fail(
                VaspFailClosedCode.ATOM_INDEX_MAP_INVALID,
                "atom map entry must be an object",
            )
...
def _reconcile_potcar_spec(
    *,
    ...
    preparations = _require_dict(manifest, "preparations")
    if (
        preparations.get("potcar_spec_sha256") != spec.sha256
        or preparations.get("potcar_metadata_hash") != spec.metadata_hash
    ):
        _fail(
            VaspFailClosedCode.POTCAR_SPEC_RECONCILIATION_MISMATCH,
            "POTCAR hashes mismatch",
        )
    return spec
...
def _recover_magmom(
    *,
    ...
    if treatment is SpinTreatment.UNPOLARIZED:
        return None
    raw_parameters = manifest.get("effective_parameters")
    if not isinstance(raw_parameters, list):
        _fail(
            VaspFailClosedCode.INPUT_MANIFEST_INVALID,
            "effective_parameters must be a list",
        )
    values: list[float] | None = None
    for raw in raw_parameters:
        if isinstance(raw, dict) and raw.get("name") == "MAGMOM":
            rendered = raw.get("value")
            if not isinstance(rendered, str):
                _fail(
                    VaspFailClosedCode.MAGMOM_UID_MISSING,
                    "spin-polarized manifest requires rendered MAGMOM",
                )
            try:
                values = [float(item) for item in rendered.split()]
            except ValueError as error:
                raise GeneratedInputReconciliationError(
                    VaspFailClosedCode.MAGMOM_UID_MISSING,
...
def _protocol_kpoint_centering(protocol: ProtocolDefinition) -> KPointCentering:
    ...
    if len(matches) != 1 or not isinstance(matches[0].value, str):
        _fail(
            ...
            "missing/invalid k-point centering",
        )
    try:
