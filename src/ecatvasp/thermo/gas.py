...
CANONICAL_IDEAL_GAS_THERMOCHEMISTRY_FORMAT = (
    "ecatvasp-canonical-ideal-gas-thermochemistry"
)
...
class GasReferenceDefinition:
    """Registry identity for one molecular reference, never a manual Gibbs number."""
    ...
    def expected_composition(self) -> tuple[tuple[str, int], ...]:
        """Return registry composition used only to validate the explicit structure."""
...
class GasRigidRotorEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        invalid_moment = any(
            ...
            for value in self.principal_moments_kg_m2
        )
        if len(self.principal_moments_kg_m2) != 3 or invalid_moment:
            raise GasThermochemistryError(
                "principal moments must be three finite non-negative values"
            )
...
def calculate_ideal_gas_thermochemistry(
    *,
    ...
    mass_by_uid: dict[AtomUid, float] = {
        item.atom_uid: item.mass_amu for item in gas_model.atomic_masses
    }
    ...
    electronic_energy = _electronic_energy(
        source_result,
        identity.electronic_energy_kind,
    )
...
def materialize_ideal_gas_thermochemistry(
    *,
    ...
    """Persist one exact gas registry reference through the existing Analysis DAG."""
...
def _validate_gas_identity(identity: ThermochemistryIdentity) -> None:
    if identity.subject_kind is not ThermochemistrySubjectKind.GAS:
        raise GasThermochemistryError(
            "ideal-gas thermochemistry requires GAS subject_kind"
        )
    if identity.gas_model is None:
        raise GasThermochemistryError(
            "ideal-gas thermochemistry requires explicit gas_model"
        )
    if identity.vibrational_policy is None:
        raise GasThermochemistryError(
            "ideal-gas thermochemistry requires vibrational policy"
        )
    ...
    if identity.gas_model.geometry_kind is GasGeometryKind.MONATOMIC:
        raise GasThermochemistryError(
            "Block 3 registry contains molecular references only; monatomic gas is unsupported"
        )
...
def _validate_reference_composition(
    *,
    ...
    if actual != expected:
        species = reference.species.value
        raise GasThermochemistryError(
            f"{species} registry reference composition differs from StructureSnapshot"
        )
...
def _electronic_energy(
    ...
    values = {
        ...
        ElectronicEnergyKind.WITHOUT_ENTROPY: (
            source_result.energies.energy_without_entropy_ev
        ),
...
def _select_gas_modes(
    *,
    ...
    if len(rotation_indices) != expected_rotations:
        geometry = gas_model.geometry_kind.value
        raise GasThermochemistryError(
            f"{geometry} gas requires exactly {expected_rotations} rotational exclusions"
        )
    ...
    for exclusion in policy.exclusions:
        ...
        if (
            ...
            raise GasThermochemistryError(
                "IMAGINARY exclusion must reference an imaginary VASP mode"
            )
    ...
    if policy.imaginary_mode_policy is ImaginaryModePolicy.REJECT_ANY:
        if imaginary:
            raise GasThermochemistryError(
                "imaginary VASP modes violate REJECT_ANY gas policy"
            )
    else:
        unexcluded = tuple(
            mode.mode_index
    ...
    if policy.low_frequency_policy is LowFrequencyPolicy.REJECT_BELOW_CUTOFF:
        if candidate_low_real:
            raise GasThermochemistryError(
                "real gas vibrations below cutoff violate selected policy"
            )
    else:
        ...
        if unexcluded_low:
            raise GasThermochemistryError(
                f"low-frequency gas vibrations require exclusions: {unexcluded_low}"
            )
    ...
    if len(accepted) > maximum_vibrations:
        raise GasThermochemistryError(
            "accepted gas modes exceed rigid-molecule vibrational degrees of freedom"
        )
    for mode_index in accepted:
        if by_index[mode_index].energy_mev <= 0.0:
            raise GasThermochemistryError(
                "accepted gas vibrational mode requires positive energy"
            )
...
def _harmonic_mode_terms(
    *,
    ...
    if not isfinite(energy_ev) or energy_ev <= 0.0:
        raise GasThermochemistryError(
            "harmonic gas mode energy must be finite and positive"
        )
    x = energy_ev / (BOLTZMANN_EV_PER_K * temperature_k)
    if not isfinite(x) or x <= 0.0:
        raise GasThermochemistryError(
            "dimensionless gas vibrational frequency must be positive"
        )
    zpe = 0.5 * energy_ev
    if x > 700.0:
        return zpe, 0.0, 0.0
    ...
    if not all(isfinite(value) and value >= 0.0 for value in values):
        raise GasThermochemistryError(
            "gas harmonic thermochemistry produced invalid components"
        )
    return values
...
def _rotation_terms(
    *,
    evidence: GasRigidRotorEvidence,
    symmetry_number: int,
    temperature_k: float,
) -> tuple[float, float]:
    ...
    if evidence.geometry_kind is GasGeometryKind.LINEAR:
        ...
        if any(moment <= 0.0 for moment in moments):
            raise GasThermochemistryError(
                "nonlinear gas requires three positive principal moments"
            )
        log_q = (
            0.5 * log(pi)
...
def _electronic_entropy(
    *,
    policy: ElectronicEntropyPolicy,
...
def _unwrapped_cartesian_positions_m(
    snapshot: StructureSnapshot,
) -> dict[AtomUid, tuple[float, float, float]]:
    ...
    for site in snapshot.sites:
        ...
        cartesian_angstrom = tuple(
            sum(
                fractional[axis] * vectors[axis][component]
                for axis in range(3)
            )
            for component in range(3)
        )
        positions[site.atom_uid] = cast(
            tuple[float, float, float],
            tuple(value * ANGSTROM_M for value in cartesian_angstrom),
        )
    return positions
...
def _validate_source_contract(
    *,
    ...
    if method_fingerprint.id != calculation.method_fingerprint_id:
        raise GasThermochemistryError(
            "MethodFingerprint differs from gas Calculation identity"
        )
...
def _write_result_artifact(
    *,
    ...
    relative = (
        Path("analyses")
        / str(analysis.id)
        / "canonical-ideal-gas-thermochemistry.json"
    )
