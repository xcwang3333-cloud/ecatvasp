...
from ecatvasp.domain import Analysis, AnalysisId, Artifact, canonical_sha256
...
def resolve_surface_reaction_source(
    *,
    ...
    if expected_subject not in {
        ...
        raise ApplicationServiceError("surface reaction source subject must be surface/adsorbate")
    bundle = store.open()
    ...
    if (analysis.tool, analysis.tool_version) != (
        ...
        raise ApplicationServiceError("surface reaction source must be canonical harmonic thermochemistry")
...
def resolve_molecular_reaction_source(
    *,
    ...
    if (analysis.tool, analysis.tool_version) == (
        ...
        receipt = _mapping(canonical.payload.get("source_receipt"), "reference source_receipt")
        raw_analysis_id = _analysis_id(receipt.get("source_analysis_id"), "source_analysis_id")
        ...
        if corrected.source_result_hash != raw.result.result_hash:
            raise ApplicationServiceError("corrected molecular reference result hash differs from raw source")
    else:
        raise ApplicationServiceError(
...
def decode_thermochemistry_result(payload: dict[str, object]) -> ThermochemistryResult:
    ...
    identity = _decode_identity(_mapping(result_raw.get("identity"), "thermochemistry identity"))
    ...
    selection = (
        ...
        else _decode_mode_selection(_mapping(selection_raw, "thermochemistry mode_selection"))
    )
    ...
    if result.result_hash != expected_hash or payload.get("result_hash") != expected_hash:
        raise ApplicationServiceError("canonical thermochemistry typed result hash differs")
    if canonical_sha256(result) != canonical_sha256(result_raw):
        raise ApplicationServiceError("canonical thermochemistry typed reopen differs from Artifact payload")
    return result
...
def decode_reference_thermochemistry_result(
    ...
    result = ReferenceThermochemistryResult(
        ...
        source_gibbs_free_energy_ev=_required_float(raw, "source_gibbs_free_energy_ev"),
    )
    expected_hash = _required_string(raw, "result_hash")
    if result.result_hash != expected_hash or payload.get("result_hash") != expected_hash:
        raise ApplicationServiceError("canonical reference thermochemistry typed result hash differs")
    if canonical_sha256(result) != canonical_sha256(raw):
        raise ApplicationServiceError("canonical reference typed reopen differs from Artifact payload")
    return result
...
def _decode_bound_gas_reference(
    ...
    if result.identity.subject_kind is not ThermochemistrySubjectKind.GAS:
        raise ApplicationServiceError("ideal-gas reaction source has non-gas thermochemistry result")
    if receipt.get("reference_content_hash") != reference.content_hash:
        raise ApplicationServiceError("gas reference content hash differs from canonical reference")
...
def _decode_identity(raw: dict[str, object]) -> ThermochemistryIdentity:
    ...
    return ThermochemistryIdentity(
        subject_kind=ThermochemistrySubjectKind(_required_string(raw, "subject_kind")),
        ...
        vibrational_policy=(
            ...
            else _decode_vibrational_policy(_mapping(vibrational_raw, "vibrational_policy"))
        ),
        gas_model=(None if gas_raw is None else _decode_gas_model(_mapping(gas_raw, "gas_model"))),
        corrections=tuple(
...
def _decode_conditions(raw: dict[str, object]) -> ThermochemicalConditions:
    ...
    return ThermochemicalConditions(
        ...
        standard_state=ThermochemicalStandardState(_required_string(raw, "standard_state")),
        pressure_pa=None if pressure is None else _finite_number(pressure, "pressure_pa"),
    )


def _decode_vibrational_policy(raw: dict[str, object]) -> VibrationalModePolicy:
    return VibrationalModePolicy(
        frequency_cutoff_cm_inverse=_required_float(raw, "frequency_cutoff_cm_inverse"),
...
def _decode_gas_mass(raw: dict[str, object]) -> GasAtomicMass:
    ...
    return GasAtomicMass(
        ...
        isotopologue_label=None if label is None else _string(label, "isotopologue_label"),
    )
...
def _decode_components(raw: dict[str, object]) -> ThermochemistryComponents:
    return ThermochemistryComponents(
        ...
        vibrational_thermal_energy_ev=_required_float(raw, "vibrational_thermal_energy_ev"),
        translational_thermal_energy_ev=_required_float(raw, "translational_thermal_energy_ev"),
        rotational_thermal_energy_ev=_required_float(raw, "rotational_thermal_energy_ev"),
        pv_ev=_required_float(raw, "pv_ev"),
        vibrational_entropy_ev_per_k=_required_float(raw, "vibrational_entropy_ev_per_k"),
        translational_entropy_ev_per_k=_required_float(raw, "translational_entropy_ev_per_k"),
        rotational_entropy_ev_per_k=_required_float(raw, "rotational_entropy_ev_per_k"),
        electronic_entropy_ev_per_k=_required_float(raw, "electronic_entropy_ev_per_k"),
        corrections=tuple(
...
def _decode_mode_selection(raw: dict[str, object]) -> ThermochemistryModeSelection:
    return ThermochemistryModeSelection(
        accepted_mode_indices=tuple(
            ...
            for item in _sequence(raw.get("accepted_mode_indices"), "accepted_mode_indices")
        ),
...
def _decode_adjustment(raw: dict[str, object]) -> GasReferenceAdjustmentIdentity:
    return GasReferenceAdjustmentIdentity(
        reference=_decode_reference(_mapping(raw.get("reference"), "adjustment reference")),
...
def _decode_reference_policy(raw: dict[str, object]) -> ReferenceCorrectionPolicy:
    ...
    return ReferenceCorrectionPolicy(
        correction=_decode_correction(_mapping(raw.get("correction"), "reference correction")),
        evidence=_decode_correction_evidence(_mapping(raw.get("evidence"), "correction evidence")),
        note=None if note is None else _string(note, "note"),
    )
...
def _decode_correction_evidence(raw: dict[str, object]) -> CorrectionEvidence:
    ...
    return CorrectionEvidence(
        ...
        artifact_id=(
            ...
            else cast(object, UUID(_string(artifact_id, "artifact_id")))
        ),
        artifact_sha256=None if artifact_sha is None else _string(artifact_sha, "artifact_sha256"),
    )
...
def _mapping(value: object, field_name: str) -> dict[str, object]:
    if not isinstance(value, dict) or any(not isinstance(key, str) for key in value):
        raise ApplicationServiceError(f"{field_name} must be a string-keyed mapping")
